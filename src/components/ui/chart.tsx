...
const ChartTooltipContent = React.forwardRef<HTMLDivElement, any>((props, ref) => {
  ...
  const payload = originalPayload as any[]; // ✅ cast to any[]
  ...
  }, [tooltipLabelProp, payload, hideLabel, labelFormatter, labelClassName, config, labelKey]);
...
const ChartLegendContent = React.forwardRef<HTMLDivElement, any>((props, ref) => {
  const { className, hideIcon = false, payload: originalPayload, verticalAlign = "bottom", nameKey } = props;
  const payload = originalPayload as any[]; // ✅ cast to any[]
  const { config } = useChart();
